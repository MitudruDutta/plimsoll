...
class MarketDataService:
    ...
    def __init__(self, demo_mode: bool = True, crisis_scenario: str | None = None):
        """
        ...
        return {
            ...
            "futures": {
                ...
                "3_month": {
                    ...
                    "expiry": (datetime.now(UTC) + timedelta(days=90)).strftime("%Y-%m-%d"),
                    "open_interest": random.randint(25000, 40000),
                },
                "6_month": {
                    "price": round(futures_6m, 2),
                    "expiry": (datetime.now(UTC) + timedelta(days=180)).strftime("%Y-%m-%d"),
    ...
    def get_freight_rates(self) -> dict:
        ...
        return {
            ...
            "time_charter_rates": {
                ...
                "panamax": {"usd_per_day": int(time_charter_rate * 0.7), "route": "Trans-Pacific"},
            },
            "spot_rates": {
                "shanghai_rotterdam": {
    ...
    def get_crisis_indicators(self) -> list[str]:
        ...
        if self.crisis_scenario == "red_sea":
            ...
            indicators.extend(
                [CrisisIndicator.FUEL_PRICE_SPIKE.value, CrisisIndicator.OPEC_PRODUCTION_CUT.value]
            )
    ...
    def _calculate_option_premium(
        ...
        """
        # Intrinsic value
        intrinsic = max(strike - spot, 0) if is_put else max(spot - strike, 0)
...
def get_market_data_service(crisis_scenario: str | None = None) -> MarketDataService:
    """
    ...
    if _market_data_service is None or crisis_scenario:
        _market_data_service = MarketDataService(demo_mode=True, crisis_scenario=crisis_scenario)
