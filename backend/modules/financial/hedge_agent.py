...
from modules.financial.market_data_service import MarketRegime, get_market_data_service
...
class FinancialHedgeAgent:
    ...
    def __init__(self, crisis_scenario: str | None = None):
        """
    ...
    def assess_risk(self, operation_params: dict) -> dict:
        ...
        fuel_var = self.market_service.calculate_var(
            exposure_usd=fuel_exposure_6m, asset_type="fuel", confidence=0.95, horizon_days=180
        )
        ...
        urgency = (
            ...
            if regime == MarketRegime.ELEVATED.value
            else "LOW"
        )
        ...
        return {
            ...
            "var_as_pct_of_exposure": (total_var / total_exposure * 100)
            if total_exposure > 0
            else 0,
            "risk_breakdown": {
                "fuel": {
    ...
    def recommend_hedging_strategy(
        self, operation_params: dict, crisis_override: bool = False
    ) -> dict:
        """
...
def get_hedge_agent(crisis_scenario: str | None = None) -> FinancialHedgeAgent:
    """
