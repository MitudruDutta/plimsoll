...
from modules.analytics.visual_risk_routes import router as visual_risk_routes
...
app = FastAPI(
    ...
    description="Maritime Operations Risk Management DJI Drone Intelligent Sales Assistant System Financial Hedging Platform", version="0.1.0"
)
...
async def chat(
    ...
    # 7. Customer(non-blocking)
    if active_conv.message_count >= 4:  # Classify only after at least 2 rounds of conversation
        bg_tasks.add_task(classify_customer_bg, request.customer_id, db)
    
    # 8. If handoff needed, create handoff record
